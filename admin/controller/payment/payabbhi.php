...
class ControllerPaymentPayabbhi extends Controller
{
    ...
    public function index()
    {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('payment/payabbhi', 'token='.$this->session->data['token'], 'SSL'),
            'separator' => ' :: ',
        );

        $data['action'] = $this->url->link('payment/payabbhi', 'token='.$this->session->data['token'], 'SSL');
    ...
    {
        if (!$this->user->hasPermission('modify', 'payment/payabbhi')) {
